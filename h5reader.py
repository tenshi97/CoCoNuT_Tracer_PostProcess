...
class hydrof():
        def __init__(self,index = 0,model = None,subdirectory = ""):
                """
...
# Read all the hdf5 files into the object.              
                ...
                for root,dirs,files in os.walk(subd):
                        for name in files:
                                if isinstance(model, str):                     
                                        if name.startswith(model+'.o') and name[-8:].isnumeric():
                                                if(subdirectory!=""):
                                                        hdf5 += [f"./{subdirectory}/{name}"]
                                                        print(hdf5)
                                                else:
                                                        hdf5 += [name]
                                else:
        ...
        def tracer_id (self, **kwargs):
                ...
                if bool(kwargs) is not False:
                        ...
                        if isinstance(index, int) and 0<=index<=self.ngroups-1:
                                ...
                                ok = False
                if ok:
                        tmp = self.steps[self.key[self.index]][self.groups[self.index]+'/trid']
                        value = np.empty (tmp.shape)
                        tmp.read_direct(value)
                        value = np.transpose(value)
                        return value
        def tracer_x (self, **kwargs):
                ok = True
                if bool(kwargs) is not False:
                        index = kwargs['index']
                        if isinstance(index, int) and 0<=index<=self.ngroups-1:
                                ...
                                ok = False
                if ok:
                        tmp = self.steps[self.key[self.index]][self.groups[self.index]+'/trx']
                        value = np.empty (tmp.shape)
                        tmp.read_direct(value)
                        value = np.transpose(value)
                        return value
        def tracer_y (self, **kwargs):
                ok = True
                if bool(kwargs) is not False:
                        index = kwargs['index']
                        if isinstance(index, int) and 0<=index<=self.ngroups-1:
                                ...
                                ok = False
                if ok:
                        tmp = self.steps[self.key[self.index]][self.groups[self.index]+'/try']
                        value = np.empty (tmp.shape)
                        tmp.read_direct(value)
                        value = np.transpose(value)
                        return value
...
# course takes much longer to do.
        ...
        def analysis (self, mem=True):
                ...
                for i in range(self.ngroups):           
                        ...
                        wl =1.0/np.sqrt(1.0-(vex**2+vey**2+vez**2)/nt.pc_cl)
                        if np.amin(gpo)<= 0.0:
                                wl[:,:,:]=1.0
                        else:
                                tau = ene*(den*wl)
                                ene=(ene+nt.pc_cl*(1.0-wl))/wl+pre*(1.0-wl**2)/(den*wl**2)
