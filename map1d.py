...
#                        xigmap +=  (rho0 + s1rho * (Xc-r3c0) + s2rho * (Yc-muc0) + s3rho * (Zc-phic0)) * np.sum(xnu0[14:nnuc-1]) * dv
    ...
    rhonew = np.maximum(rhonew, 1e-15)
    for i in range(nnuc):
...
# Assuming slightly proton-rich ejecta
    ...
    xartis[:,:,:,28] = xni56 + xignew *  8e-15 # Ni56, Ni58
...
# Normalise mass fractions
    ...
    for ii in range(nx):
        ...
        f.write ("%d %16.8e %16.8e %16.8e %16.8e %16.8e %16.8e %16.8e \n" % (ij, velocity, np.log10(xartis[ii,0,0,0]), xignew[ii,0,0], xni56[ii,0,0], xco56[ii,0,0], xfe52[ii,0,0], xcr48[ii,0,0]))
        out = " ".join(("%16.8e " %dat) for dat in tuple(xartis[ii,0,0,1:]))
        g.write(("%d " % ij) + out + " \n")

    f.closed
    g.closed

    # write_file(model, nx, nz, x1, z1, x, z, xartis, xignew, xni56, xco56, xfe52, xcr48)
