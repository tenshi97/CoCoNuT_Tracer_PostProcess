...
class TracerReader_3D:
    ...
    def XTimeSeriesForOneTracer(self,start=1,end=-1,scatter=False,scale_log=False):
        tracerX_plot = []
        time_plot = []
        ...
        plt.savefig(f"XTS_{self.modelname}_tracer_{self.id}.png")
        plt.close()
    ...
    def YTimeSeriesForOneTracer(self,start=1,end=-1,scatter=False):
        ...
        plt.savefig(f"YTS_{self.modelname}_tracer_{self.id}.png")
        plt.close()

    def ZTimeSeriesForOneTracer(self,start=1,end=-1,scatter=False):
        tracerY_plot = []
        time_plot = []
        plt.figure()
        if(start<0 or start>=self.n):
        ...
        for i in range(start,end+1):
            self.set_index(i)
            if(scatter):
                plt.scatter(self.timepoints[i],self.getZ(),c="blue")
                continue
            tracerY_plot.append(self.getZ())
            time_plot.append(self.timepoints[i])
        if(not scatter):
            plt.plot(time_plot,tracerY_plot)
        plt.title(f"Phi Coordinates of Tracer {self.id} Time Series")
        plt.savefig(f"ZTS_{self.modelname}_tracer_{self.id}.png")
        plt.close()
